...
We've seen this output before, but let's review because if you are using scatterplots to make a point, you should do this. First, note the Min and Max residual at the top. A team has underperformed the model by 13 points, and a team has overperformed it by 14. The median residual, where half are above and half are below, is just slightly under the fit line. Close here is good. 

Next: Look at the Multiple R-squared value. What that says is that 42 percent of a team's scoring output can be predicted by their third down conversion percentage. This is just one year, so that's a little low. If we did this with more years, that would go up. 
...
The lack of a shape here -- the seemingly random nature -- is a good sign that a linear model workds for our data. 
...
* Our min and max go from -16 to positive 19
* Our multiple R-squared is ... .019. Not much at all. 
* Our p-value is ... .12, which is GREATER than .05, which means it's entirely possible the predictive power of our correlation is completely random. 
...
So our model says Alabama *should* only be scoring 29 points per game given how many penalty yards per game, but they're really scoring 48. Oy. What happens if we plot those residuals? 
